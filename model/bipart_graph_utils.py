...
import numpy as np
from bipart_lsh import get_negs_by_lsh
from io import open
import os
import itertools
import sys
from homo_main import *
import pandas as pd
...
class GraphUtils(object):
    ...
    def construct_training_graph(self, filename=None):
        ...
        with open(filename, encoding="UTF-8") as fin:
            ...
            while line:
                ...
                if self.edge_dict_v.get(item) is None:
                    ...
                    #print('self.edge_dict_v[item]:',self.edge_dict_v[item])
        ...
        self.node_u = sorted(self.node_u)
        self.node_v = sorted(self.node_v)
        #self.node_u.sort()
        #self.node_v.sort()
    ...
    def homogeneous_graph_random_walks_similarity_based(self, percentage, maxT, minT,target_type):

        R_file='..\data\{}_file\{}_interactions\{}_admat_dgc_mat_2_line.txt'.format(target_type,target_type,target_type)
        # "Name of the file containg drug similarties (homogeneous) file names"
        D_sim_file = '..\data\{}_file\{}_homogeneous\{}_D_SimLine_files.txt'.format(target_type,target_type,target_type)
        # "Name of the file containg target similarties (homogeneous) file names"
        T_sim_file = '..\data\{}_file\{}_homogeneous\{}_T_SimLine_files.txt'.format(target_type,target_type,target_type)
        # "Number of neighbors similarity fusion. Default: 3"
        K_SNF = 3
        # "Number of iteration for similarity fusion. Default: 10"
        T_SNF = 10
        D_SNF, t_SNF, dDs, dTs = run_DDR(R_file, D_sim_file, T_sim_file, K_SNF, T_SNF, target_type)
        D_SNF=np.array(D_SNF)
        t_SNF=np.array(t_SNF)
        D_sim_list,T_sim_list=[],[]
        for i in dDs:
            for j in dDs:
                row=dDs[i]
                col=dDs[j]
                D_sim_list.append(['u' + i, 'u' + j, D_SNF[row, col]])
        for i in dTs:
            for j in dTs:
                row = dTs[i]
                col = dTs[j]
                T_sim_list.append(['i' + i, 'i' + j, t_SNF[row, col]])
        D_sim_list=pd.DataFrame(D_sim_list).sort_values(by=2, axis=0, ascending=False, inplace=False)[:]
        T_sim_list=pd.DataFrame(T_sim_list).sort_values(by=2, axis=0, ascending=False, inplace=False)[:]
        #print(D_sim_list)
        D_sim_list.to_csv(self.fw_u,sep='\t',header=None,index=False)
        T_sim_list.to_csv(self.fw_v, sep='\t', header=None, index=False)
    ...
    def get_random_walks_restart(self, datafile, hits_dict, percentage, maxT, minT):
        ...
        G = bipart_graph.load_edgelist(datafile, undirected=True)

        print("number of nodes: {}".format(len(G.nodes())))
        print("walking...")

        walks = bipart_graph.build_deepwalk_corpus_random(G, hits_dict, percentage=percentage, maxT = maxT, minT = minT, alpha=0)
        print("walking...ok")
        return G, walks
    ...
    def homogeneous_graph_random_walks_for_large_bipartite_graph_without_generating(self, datafile, percentage, maxT, minT, alpha):
        self.G_u, self.walks_u = self.get_random_walks_restart_for_large_bipartite_graph_without_generating(datafile, self.authority_u, percentage=percentage, maxT=maxT, minT=minT, alpha=alpha, node_type='u')
        self.G_v, self.walks_v = self.get_random_walks_restart_for_large_bipartite_graph_without_generating(datafile, self.authority_v, percentage=percentage, maxT=maxT, minT=minT, alpha=alpha, node_type='i')


    def get_random_walks_restart_for_large_bipartite_graph(self, matrix, hits_dict, percentage, maxT, minT):
        G = bipart_graph.load_edgelist_from_matrix(matrix, undirected=True)
        print("number of nodes: {}".format(len(G.nodes())))
        print("walking...")
        walks = bipart_graph.build_deepwalk_corpus_random(G, hits_dict, percentage=percentage, maxT = maxT, minT = minT, alpha=0)
        print("walking...ok")
    ...
    def get_random_walks_restart_for_large_bipartite_graph_without_generating(self, datafile, hits_dict, percentage, maxT, minT, alpha, node_type='u'):
        if datafile is None:
            datafile = os.path.join(self.model_path,"rating_train.dat")
        G = bipart_graph.load_edgelist(datafile, undirected=True)
        ...
        print("walking...")
        walks = bipart_graph.build_deepwalk_corpus_random_for_large_bibartite_graph(G, hits_dict, percentage=percentage, maxT = maxT, minT = minT, alpha=alpha, node_type=node_type)
    ...
    def get_context_and_fnegatives(self,G,walks,win_size,num_negs,table):
        ...
        if isinstance(G, bipart_graph.Graph):
            node_list = G.nodes()
    ...
    #A.dot(AT),self.fw_u, index_row,index_row)
